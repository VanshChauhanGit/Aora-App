...
export const unlikeVideo = async (videoId, userId) => {
  try {
    ...
    if (!Array.isArray(video.likes)) {
      return { success: false, message: "Invalid likes data" };
    }

    // Check if the user has already liked the video
    const userLiked = video.likes.some((like) => like.$id === userId);
    if (!userLiked) {
    ...
    return { success: false, message: "Failed to unlike video" };
  }
};
...
export const isVideoLikedByUser = async (videoId, userId) => {
  try {
    const video = await databases.getDocument(
      config.databaseId,
      config.videoCollectionId,
    ...
    const isLiked = video.likes.some((like) => like.$id === userId);
    return isLiked;
  } catch (error) {
